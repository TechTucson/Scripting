...
FOCUS = 0  # 0=left, 1=right, 2=bottom
...
def draw_right(win):
    ...
    win.addstr(3, 2, STATE["llm_prompt"][:win.getmaxyx()[0]-6])
    mid = win.getmaxyx()[0] // 2
    win.addstr(mid, 2, "RESPONSE:")
    win.addstr(mid+1, 2, STATE["llm_response"][:win.getmaxyx()[0]-mid-4])
    win.refresh()
...
def main(stdscr):
    ...
    # --- Initial setup ---
    STATE["hex_url"] = input("Hexstrike URL [default ok]: ") or DEFAULT_HEXSTRIKE_URL
    STATE["ollama_url"] = input("Ollama URL [default ok]: ") or DEFAULT_OLLAMA_URL
    STATE["tools"] = discover_tools()

    models = requests.get(f"{STATE['ollama_url']}/api/tags").json()["models"]
    for i, m in enumerate(models, 1):
        print(f"{i}. {m['name']}")
    STATE["model"] = models[int(input("Select model: ")) - 1]["name"]
