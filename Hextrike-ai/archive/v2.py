...
def print_scope_summary():
  ...
  def prompt_for_target():
    """Prompt user for target and target type with defaults"""
    
    # Prompt for target
    user_target = input(
        f"Enter target domain [{SCOPE['target']}]: "
    ).strip()
    
    if user_target:
        SCOPE['target'] = user_target
    ...
    user_target_type = input(
        f"Enter target type {valid_target_types} "
        f"[{SCOPE['target_type']}]: "
    ).strip().lower()

    if user_target_type:
        if user_target_type in valid_target_types:
            SCOPE['target_type'] = user_target_type
        else:
            print(
                f"⚠️  Invalid target type '{user_target_type}', "
                f"defaulting to '{SCOPE['target_type']}'"
            )

def ai_guided_pentest():
    """Main AI-guided penetration testing function"""
  
    # Prompt user for runtime target configuration
    prompt_for_target()    
